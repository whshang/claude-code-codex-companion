...
### 双客户端兼容
- 自动识别 Claude Code / Codex / OpenAI 格式；必要时在 `/responses` ↔ `/chat/completions` ↔ `/messages` 间转换。
- 单端口同时兼容两种客户端；双 URL（`url_anthropic` + `url_openai`）可按格式分流。
- 学习并持久化 `supports_responses` 与 `openai_preference`，避免每次重试。
- 详见 [OpenAI 集成设计](docs/OpenAI集成设计_OpenAI_Integration_Design.md)。
