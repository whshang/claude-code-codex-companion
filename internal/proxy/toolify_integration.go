...
func extractAssistantTextForToolDetect(body []byte, endpointFormat string) string {
	...
	if format == "openai" || format == "" {
		if text := strings.TrimSpace(collectCodexResponseText(m)); text != "" {
			return text
		}
					...
					if contentArr, ok := msg["content"].([]interface{}); ok {
						...
						for _, contentItem := range contentArr {
							if contentMap, ok := contentItem.(map[string]interface{}); ok {
								if txt, ok := contentMap["text"].(string); ok && txt != "" {
									if b.Len() > 0 {
										b.WriteString("\n")
									}
									b.WriteString(txt)
...
func collectCodexResponseText(m map[string]interface{}) string {
	var parts []string
	appendText := func(val string) {
	...
	collect = func(v interface{}) {
		...
		case map[string]interface{}:
			if text, ok := vt["text"]; ok {
				collect(text)
			}
			if value, ok := vt["value"]; ok {
				collect(value)
			}
			if outputText, ok := vt["output_text"]; ok {
				collect(outputText)
			}
			if content, ok := vt["content"]; ok {
				collect(content)
			}
			if message, ok := vt["message"]; ok {
				collect(message)
			}
			for key, item := range vt {
				if key == "text" || key == "value" || key == "output_text" || key == "content" || key == "message" {
					continue
				}
				collect(item)
			}
