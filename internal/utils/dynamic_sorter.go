...
func (des *DynamicEndpointSorter) SortAndApply() {
    ...
    // 锁定并重新分配所有端点的优先级（数字越大优先级越高）
	des.mu.Lock()
	defer des.mu.Unlock()

    // 重新分配端点的优先级：先高优先级给可用的启用端点，再给不可用的启用端点，最后给禁用端点
	...
	for _, ep := range disabledEndpoints {
        ...
        if ep.GetPriority() != currentPriority {
            log.Printf("🔒 禁用端点 %s 优先级从 %d 调整为 %d", ep.GetName(), ep.GetPriority(), currentPriority)
            ep.SetPriority(currentPriority)
        }
        currentPriority--
	}
