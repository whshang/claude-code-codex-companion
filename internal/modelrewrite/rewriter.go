...
func (r *Rewriter) RewriteRequestWithTags(req *http.Request, modelRewriteConfig *config.ModelRewriteConfig, endpointTags []string, clientType string) (string, string, error) {
	...
	if hasExplicitRules {
		...
		if (clientType == "claude-code" || isHealthCheck) && !strings.HasPrefix(originalModel, "claude") {
			// Claude Code 客户端或健康检查：非 Claude 模型转为 Claude 默认模型
			defaultModel = "claude-sonnet-4-20250929"
			shouldApplyImplicit = true
		} else if (clientType == "codex" || isHealthCheck) && !strings.HasPrefix(originalModel, "gpt") {
			// Codex 客户端或健康检查：非 GPT 模型转为 GPT 默认模型
			defaultModel = "gpt-5"
			shouldApplyImplicit = true
